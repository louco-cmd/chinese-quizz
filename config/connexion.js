...
passport.deserializeUser(async (id, done) => {
  try {
    ...
    const res = await pool.query(
      "SELECT id, email, name, is_admin, country, tagline, quiz_direction, interface_lang, onboarding_done, ghost_mode, notifications_enabled, word_review_enabled, role FROM users WHERE id = $1",
      [id]
    );
