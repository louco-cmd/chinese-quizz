...
// Bonus de bienvenue de l'INVITÉ (en plus du solde de départ standard) : un compte
// parrainé démarre avec 50 coins de plus que les autres. Versé en même temps que la
// récompense du parrain (même verrou email-vérifié → anti-farming).
...
async function rewardPendingReferral(userId) {
  ...
  try {
    ...
    if (upd.rowCount === 1) {
      ...
      // Bonus de bienvenue de l'invité (idempotent via le même verrou FALSE→TRUE).
      await addTransaction(client, userId, INVITEE_BONUS, 'referral', 'Welcome bonus (referred)');
    }
...
module.exports = { rewardPendingReferral, REFERRAL_REWARD, INVITEE_BONUS };
