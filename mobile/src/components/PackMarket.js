...
// Grille 2 colonnes des packs JiaStore + popup d'achat. Réutilisable (store,
// onboarding). `extraTile` = un nœud injecté dans la grille à `extraTileAt`
// (index 0-based ; ex. 1 = 2e tuile = haut de la colonne de droite).
        ...
        renderItem={({ item }) =>
          item._spacer ? <View style={{ flex: 1 }} />
            : item._extra ? extraTile
              : <PackCard pack={item} onPress={openPack} />
        }
        ListEmptyComponent={
          loading ? (
